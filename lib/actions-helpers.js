  ...
  toEmbed &&
    [].concat(toEmbed).forEach((externalResource) => {
      if (db.get(externalResource).value) {
        const query = {};
        query[foreignKeyId(name, opts)] = resource.id;
  ...
  toExpand &&
    [].concat(toExpand).forEach((innerResource) => {
      ...
      if (db.get(plural).value()) {
        const prop = foreignKeyId(innerResource, opts);
...
function foreignKeyId(resourceName, opts = {}) {
  {
...
module.exports = { embed, expand, cleanQuery, foreignKeyId };
