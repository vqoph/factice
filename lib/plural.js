...
const { embed, expand, cleanQuery, foreignKeyId } = require('./actions-helpers');
...
module.exports = (db, name, opts) => {
  ...
  function list(
    { query, protocol, originalUrl, headers: { host } },
    { parentResourceName, parentId }
  ) {
    // Resource chain
    let chain = db.get(name);
    if (parentId) {
      const parentResKeyId = foreignKeyId(parentResourceName, opts);
      chain = chain.filter(
        (item) =>
          item[parentResKeyId] &&
  ...
  // POST /name
  function create({ body }, { parentResourceName, parentId, opts }) {
    if (parentId) {
      body[foreignKeyId(parentResourceName, opts)] = parentId;
    }
  ...
  return [
    { key: name, get: list, post: create, parentResources },
    {
      key: `${name}/:id`,
      get: show,
      put: update,
